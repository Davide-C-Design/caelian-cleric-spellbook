...
const INNATE_SPELLS=[{n:"Light",usesPerDay:1,note:"Aasimar racial spell-like ability. Functions like the Light spell; always available, never consumes a prepared slot."}];
let tab="cleric", selected=null;
let state={prepared:[],favorites:[],character:{level:5,wisdom:23},innateUsed:{}};
...
function innateSpellData(name){return allSpells("cleric").find(s=>s.n===name)||{n:name,l:0,s:"",d:"",source:"cleric"};}
function innateRemaining(entry){return entry.usesPerDay-(state.innateUsed[entry.n]||0);}
function toggleInnateUse(entry){const used=state.innateUsed[entry.n]||0;state.innateUsed[entry.n]=used>=entry.usesPerDay?0:used+1;save();render();}
function innateCardHTML(entry,mobile){const remaining=innateRemaining(entry),spell=innateSpellData(entry.n);return `<div class="slotcard ${remaining<=0?"over":""}"><h3>${esc(entry.n)} <span class="count">Innate</span></h3><p>${esc(entry.note)}</p><div class="used">${entry.usesPerDay-remaining} / ${entry.usesPerDay} used today</div><div class="actionrow"><button class="action secondary" data-innate="${esc(entry.n)}" data-scope="${mobile?"mobile":"main"}">${remaining>0?"Mark used":"Mark available"}</button>${spell.d?`<button class="action secondary" data-innateview="${esc(entry.n)}">View spell</button>`:""}</div></div>`;}
function renderInnate(){
 const html=INNATE_SPELLS.map(entry=>innateCardHTML(entry,false)).join(""),mobileHtml=INNATE_SPELLS.map(entry=>innateCardHTML(entry,true)).join("");
 const main=document.getElementById("innateSpells"),mobile=document.getElementById("mobileInnateSpells");
 if(main)main.innerHTML=html; if(mobile)mobile.innerHTML=mobileHtml;
 [main,mobile].forEach(node=>{if(!node)return;node.querySelectorAll("[data-innate]").forEach(button=>button.onclick=()=>{const entry=INNATE_SPELLS.find(e=>e.n===button.dataset.innate);toggleInnateUse(entry);});node.querySelectorAll("[data-innateview]").forEach(button=>button.onclick=()=>showSpellModal(innateSpellData(button.dataset.innateview)));});
}
...
function renderToday(el){
 ...
 const todayInnate=document.getElementById("todayInnate"); if(todayInnate){todayInnate.innerHTML=INNATE_SPELLS.map(entry=>innateCardHTML(entry,false)).join("");todayInnate.querySelectorAll("[data-innate]").forEach(button=>button.onclick=()=>{const entry=INNATE_SPELLS.find(e=>e.n===button.dataset.innate);toggleInnateUse(entry);});todayInnate.querySelectorAll("[data-innateview]").forEach(button=>button.onclick=()=>showSpellModal(innateSpellData(button.dataset.innateview)));}
 document.getElementById("clearPrepared").onclick=()=>{state.prepared=[];state.innateUsed={};save();render();if(selected)openSpell(selected);};
}
